...
model = load_model('/Users/zhaolei/Library/Mobile Documents/com~apple~CloudDocs/zhleicode/MLStudy/cats_and_dogs_small_2.h5')
model.summary()

#Preprocessing a single image
img_path = '/Users/zhaolei/Desktop/dataset/cats_and_dogs_small/test/cats/cat.1700.jpg'
...
for layer_name, layer_activation in zip(layer_names, activations):
    ...
    n_cols = n_features
    display_grid = np.zeros((size * int(n_cols / images_per_row), images_per_row * size))
    #循环每一个filter。
...
#                     row * size : (row + 1) * size] = channel_image
        display_grid[int(col/16) * size : (int(col/16)+1) * size,
                     int(col % 16) * size : (int(col % 16) + 1) * size] = channel_image
